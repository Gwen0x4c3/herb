...
public class TextController {
     ...
     */
    @GetMapping("/list")
    @ApiOperation(value = "获取数据列表", notes = "获取text数据列表。\n" +
            "page--第几页，从1开始\nlimit--一页多少条数据\nkey--关键词，可选参数")
    public R list(@RequestParam("page") Long curPage, @RequestParam("limit") Long limit, String key){
        PageUtils page = textService.queryPage(curPage, limit, key);
     ...
     */
    @GetMapping("/info/{id}")
    @ApiOperation(value = "获取指定id的数据", notes = "获取指定id的text数据。\n" +
            "id--数据的id，使用url传参")
     ...
     */
    @PostMapping("/save")
    @ApiOperation(value = "添加数据", notes = "添加text数据}")
     ...
     */
    @PostMapping("/update")
    @ApiOperation(value = "修改数据", notes = "根据上传数据中id修改text数据}")
     ...
     */
    @PostMapping("/delete")
    @ApiOperation(value = "删除数据", notes = "删除id在所传数组中的text数据}")
